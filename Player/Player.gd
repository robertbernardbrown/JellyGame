...
func _ready():
	...
	mat.shader = load("res://Player/flash.gdshader")
	anim.material = mat
...
func _update_energy_bar(delta: float):
	...
	var bar_val = usable * 100.0
	_energy_progress_bar.value = max(5.0, bar_val) if bar_val > 0.0 else 0.0

	# Pulse bar toward red when energy is low
	if energy < 0.35:
		var low = 1.0 - smoothstep(0.05, 0.35, energy)
		var pulse = sin(_time * lerp(3.0, 7.0, low)) * 0.5 + 0.5
		var shift = low * pulse * 0.75
		_energy_progress_bar.modulate = Color(1.0, 1.0 - shift, 1.0 - shift)
	else:
		_energy_progress_bar.modulate = Color.WHITE
